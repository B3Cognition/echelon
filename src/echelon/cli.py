...
_CARD_INNER = 52  # characters between ╭ and ╮
...
def _banner(
    ...
    fill = "─" * max(0, _CARD_INNER - len(prefix))
    print(f"\n╭{prefix}{fill}╮", flush=flush)
    if subtitle:
        body = f"  {subtitle}"
        if len(body) > _CARD_INNER:
            body = body[: _CARD_INNER - 1] + "…"
        print(f"│{body.ljust(_CARD_INNER)}│", flush=flush)
    print(f"╰{'─' * _CARD_INNER}╯\n", flush=flush)

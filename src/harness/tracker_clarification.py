"""Managed Tracker bridge to the existing human-input and publication owners.
...
from harness.discovery_producer import SOURCE_FIELDS, tracker_rounds, identity_spec_tree
...
def _record(decision):
    ...
    if (checked != decision or checked["status"] != "resolved"
            or checked["source_phase"] != "phase1-tracker" or checked["resolution_handler"] != "clarification_resume"
...
def previous_records(state, operation_id):
    rounds = tracker_rounds(state)["rounds"]
    ...
    while True:
        ...
        if resolution is None:
            break
        records.append(_record(resolution["decision"]))
        operation_id = row["predecessor"]
    return tuple(reversed(records))


def _candidate(sources, spec_path, run_path, decision, previous):
    ...
    candidate = prepare_clarification_candidate(decision=_record(decision), previous=previous,
...
def prepare(root, state_store, *, state, resolved, completion_id):
    ...
    _, _, _, history, _, _, original, source_inputs = _capture(root, state_store, store, selected,
        ...
        producer="tracker", source_completion=source)
    previous = previous_records(state, operation["binding"]["operation_id"])
    candidate, texts, before = _candidate(original, selection["spec_path"],
        state_store.squad_dir.relative_to(root).as_posix(), resolved, previous)
    spec_path = selection["spec_path"]
    report = "feature-policy-reconciliation.md"
    artifacts = tuple(CandidateArtifact(name, artifact_roles("tracker")[name], text,
    ...
    preview = store.preview_identity_candidate(spec_id=selection["spec_id"], artifacts=artifacts,
        scope=IdentityEditScope(scope_paths, (), scope_paths), operations=())
    ...
    recovery = dict(version=5, producer="tracker", completion_id=completion_id, operation=operation,
...
def decode_clarification_binding(publication, request, recovery, completion_id, state):
    ...
    _require(type(recovery["version"]) is int and recovery["version"] == 5 and recovery["producer"] == "tracker"
    ...
    prepared, texts, _ = _candidate(sources, spec_path, run_path, recovery["resolution"], previous)
    ...
    if state is not None:
        ...
        _require(bootstrap is not None and state["managed_identity"] == genesis
            ...
            and operation_from_state(state, "tracker", operation_id=selected["operation_id"]) == recovery["operation"]
            and previous_records(state, selected["operation_id"]) == previous)
        decisions = [state.get("blocked_decision"), *(row["resolution"]["decision"]
            for row in tracker_rounds(state)["rounds"].values() if row["resolution"] is not None)]
        ...
        if successors:
            ...
            _require(rounds["active"] == selected["operation_id"] and state["blocked_decision"]["id"] == current["id"]
                and dispatch.get("phase_id") == "phase1-tracker" and dispatch.get("post_dispatch_complete") is True
    ...
    candidate["route"] = "phase1-what" if json.loads(prepared.reconciliation_json)["requires_repair"] else "phase1-tracker"
...
def require_parent(state, binding, store):
    ...
    _require(parent is not None and parent.producer == "tracker" and not parent.clarification

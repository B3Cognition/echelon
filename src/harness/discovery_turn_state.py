...
def discovery_turns_from_state(state, producer="discovery", *, operation_id=None, repair_unit=None):
    marker = producer_component(state, producer, "turns", operation_id=operation_id, repair_unit=repair_unit)
    if marker is None:
        return None
    ...
    if (bootstrap is None or "managed_identity" not in state or type(marker) is not dict
            ...
            or marker["operation_id"] != producer_operation_id(state, producer, operation_id, repair_unit=repair_unit)

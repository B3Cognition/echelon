...
def discovery_turns_from_state(state):
    if DISCOVERY_TURNS_KEY not in state:
        return None
    marker = state[DISCOVERY_TURNS_KEY]
    ...
    if (bootstrap is None or "managed_identity" not in state or type(marker) is not dict
            ...
            or marker["operation_id"] != bootstrap["selection"]["operation_id"]

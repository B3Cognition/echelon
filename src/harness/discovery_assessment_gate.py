...
def evaluate_feasibility_gate(*, sources, root, spec_path, config,
    ...
    entry = governance.get("artifacts", {}).get("feasibility", {})
    _require(type(entry) is dict)
    # No configured path can redirect this report-only producer into a source.
    for key, default in (("path", "feasibility.md"), ("report", "feasibility-structural-report.json")):
        _require(str(entry.get(key) or default).strip() == default)
    template = entry.get("template")
    _require(not template or template == "feasibility-template.md")
    ...
    template_bytes = files.get(".echelon/runtime/templates/feasibility-template.md")
    references = {}
    ...
    return evaluate_captured_governance_structural_gate(artifact_key="feasibility",
        spec_dir=Path(root) / spec_path, governance_config=config,
        artifact_text=documents.get("feasibility.md"), reference_texts=references,
...
def decode_feasibility_gate_binding(publication, request, recovery, completion_id, state):
    ...
    if report is not None:
        writes[spec.path + "/feasibility-structural-report.json"] = (_json(report) + "\n").encode("ascii")
...
def prepare_feasibility_gate_publication(root, state_store, *, completion_id, max_iterations):
    ...
    _require(state["phase"] == "phase2-feasibility-structural" and state["status"] == "running"
    ...
    writes = {} if report is None else {spec.path + "/feasibility-structural-report.json": (_json(report) + "\n").encode("ascii")}
    modes = {item.path: item.image.mode for item in spec.files}
    ...
    try:
        ...
        decode_feasibility_gate_binding(dict(kind="external", marker=publication.marker.to_dict()), request, recovery, completion_id, state)
        ...
        provisional.discard()
    return PreparedFeasibilityGate(publication, sources, request, SquadAgentResult(exit_code=0,

...
from harness.discovery_operation_state import DISCOVERY_OPERATION_KEY
...
def require_legacy_identity_execution(
    *, project_root: Path, run_dir: Path, state: dict,
    ...
    try:
        if type(state) is not dict or any(key in state for key in (
                "managed_identity", BOOTSTRAP_KEY, DISCOVERY_TURNS_KEY, DISCOVERY_OPERATION_KEY)):

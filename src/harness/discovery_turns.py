    ...
    try:
        ...
        with DiscoveryReceiptFile(state_store.squad_dir, "discovery-turns", producer=producer, repair_unit=repair_unit,
            ...
            for name in ("producer", "reviewer"):
                ...
                role_name = producer_role(producer, name)
                role = loader.load_subagent(role_name)
                if role is None or role.frontmatter.get("name") != role_name or not role.body.strip():
            ...
            if producer in {"tracker", "why1", "constitution", "what", "why2", "lexicon", "feasibility"} or repair_unit is not None:

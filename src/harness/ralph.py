...
_TOOL_ACCESS_LINE_RE = re.compile(
    ...
    r"\b(?:cat|rg|grep|find|ls|sed|head|tail|less|more|awk|nl|wc|file|stat|strings|hexdump|xxd|od|cmp|diff|git|gh|curl|wget|http|https|jq|yq|dasel|xmllint|open|vim|vi|nano|emacs|code|tee|touch|mkdir|rm|rmdir|chmod|chown|chgrp|ln|install|truncate|dd|patch|cp|mv|rsync|ditto|tar|zip|unzip|gzip|gunzip|python|python3|node|deno|ruby|perl)\s+"
    r")",
    re.IGNORECASE,
)

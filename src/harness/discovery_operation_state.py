...
def validate_binding(state, binding, producer="discovery", *, operation_id=None, repair_unit=None):
    ...
    if (type(intent) is not dict or intent.get("kind") not in ({"align"} if producer == "alignment" else {"strategize"} if producer == "strategy" else {"assess"} if producer == "feasibility" else {"derive"} if producer == "lexicon" else {"specify"} if producer == "what" else {"validate"} if producer == "why2" else {"create", "repair"} if producer == "discovery" else {"constitute"} if producer == "constitution" else {"challenge"} if producer == "why1" else {"track"} if producer == "tracker" else {"synthesize"})

...
def validate_binding(state, binding, producer="discovery", *, operation_id=None):
    ...
    if (type(intent) is not dict or intent.get("kind") not in ({"create", "repair"} if producer == "discovery" else {"challenge"} if producer == "why1" else {"track"} if producer == "tracker" else {"synthesize"})

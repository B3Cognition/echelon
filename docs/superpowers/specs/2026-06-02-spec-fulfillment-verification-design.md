...
- Add a short reopen note to `reopen-{n}.md`.
...
Output: `{verify_run_dir}/requirement-audit.md`.
...
Always uses verification-local CodeGraph output generated by the verify-spec workflow. Reads the refreshed summary first. Reads the refreshed full graph only for symbol-level trace or impact evidence.
...
For each actionable gap, the reopen phase appends a small test-first task sequence to `tasks.md`: add a failing test, implement the missing or deviated behavior, then rerun `echelon verify-spec` to update fulfillment evidence.

# Captured graph input parsers implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox syntax for tracking.

**Goal:** Let the graph's existing requirement, deferral and fulfillment readers parse supplied captured text without reopening mutable files.

**Architecture:** Extract three existing parsing bodies into pure text entry points and make the existing filesystem readers thin I/O wrappers over the same bodies. Preserve all legacy interpretation and output records. This is a compatibility extraction for subsequent captured-source graph composition, not a managed acceptance validator.

**Tech Stack:** Existing Python dataclasses, JSON and Markdown readers; offline pytest compatibility tests.
...
- Existing labels remain exactly as published.
...
- Preserve the existing readers' interpretation, dataclasses, field defaults, ordering and filesystem error behavior; do not substitute a new managed schema, identity grammar or semantic authority.

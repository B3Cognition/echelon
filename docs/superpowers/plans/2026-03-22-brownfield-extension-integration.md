...
| Modify | `commands/squad.run.md` | Fix Section 15 error table row + line ~619 advisory text |
| Modify | `commands/squad.build.md` | python → python3 (already staged, just needs commit) |

---

## Task 1: Commit existing python3 fixes

The `python` → `python3` change in `commands/squad.run.md` (5 occurrences) and `commands/squad.build.md` (2 occurrences) is already in the working tree. Commit it standalone before touching anything else.

**Files:**
- Modify: `commands/squad.run.md` (already done)
- Modify: `commands/squad.build.md` (already done)

- [ ] **Step 1.1: Verify the diff is what we expect**

```bash
git diff commands/squad.run.md | grep "^[-+].*python"
git diff commands/squad.build.md | grep "^[-+].*python"
```
...
```bash
git add commands/squad.run.md commands/squad.build.md
...
## Task 8: Fix squad.run.md — Section 15 error table + line ~619 advisory text
...
```bash
grep -n "spec-kit-reverse-eng.*suggest running" commands/squad.run.md
```

Expected: no output (old text gone).

```bash
grep -n "brownfield-index.md is present" commands/squad.run.md
```

Expected: 1 hit (new text at line ~619).

```bash
grep -n "DISCOVER falls back to greenfield mode" commands/squad.run.md
```

Expected: no output (old error table row gone).

```bash
grep -n "COMMANDER treats as empty-extensions" commands/squad.run.md
```
...
1. `commands/squad.run.md` — user invokes the squad run command (note: `squad.run.md` does NOT call PROSPECTOR directly; COMMANDER handles all agent dispatch, including PROSPECTOR)
...
|------|------|-------------|
| `commands/squad.run.md` | 1, 8 | python3 fix + error table + advisory text |
| `commands/squad.build.md` | 1 | python3 fix |

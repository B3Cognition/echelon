# Coherent identity candidate preview implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox syntax for tracking.

**Goal:** Validate a captured candidate and its exact proposed journal operations against one retained authority snapshot, returning proposed history only for a structurally coherent candidate.

**Architecture:** One new opt-in IdentityStore reader composes existing candidate/source checks, journal child decoding/planning and retained-history overlay within one query-only transaction. It derives lifecycle and binding requests exclusively from the canonical child operations, ties issue occurrences to captured report contexts and retains existing diagnostic and evidence interpretation. This is a pre-intent observation, not semantic authorization or publication.

**Tech Stack:** Existing SQLite authority and typed candidate/operation/history records; one focused integration module, thin public reader, real on-disk tests.
...
- Preserve existing candidate scope/definition/reference/issue interpretation, native journal operation order/codec, snapshot wire and v1/v2/v3 publication compatibility.
- A clean structural preview is not semantic verification, physical source freshness, accepted-source ownership, dependency completeness, a lease, a receipt or permission to publish.

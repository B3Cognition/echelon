...
**Target ancestor lifetime clarification:** Retain every currently existing target ancestor's descriptor/entry association under the original owner for the entire publish_sources invocation, not just one mutation. Join newly created/first securely traversed target parents to that owner during the operation, before their temporary traversal descriptors are closed, so replacement before a later capture cannot be silently adopted. Retain existing-directory bindings, not global missing-entry or old membership pins that would reject authorized parent creation/target changes. Revalidate the retained chain at the existing boundaries and through mutation helpers. This is invocation-local inode association: an original serialized snapshot has no inode authority across separate calls or recovery, and read-only non-target source directories keep the plan's checked-capture semantics. Do not add a lock, persistent inode format, or blanket file-inode lifetime policy. Add a real same-content/mode target-ancestor replacement regression between operation/capture boundaries before implementing this strengthening.

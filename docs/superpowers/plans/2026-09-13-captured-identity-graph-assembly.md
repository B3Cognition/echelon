# Captured identity graph assembly implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox syntax for tracking.

**Goal:** Assemble an identity-projected graph from one coherent set of projected source images and explicit captured domain observations, without live reads or publication.

**Architecture:** Compose the existing pure local, memory and RE transformations in legacy domain order, check every supplied graph source against the shared image table, and apply the existing retained-history projection last. The function emits existing graph records and wire bytes; it does not select dependencies, authenticate observations, validate semantic edits, seal or publish files.

**Tech Stack:** Existing projected-source carrier/manifest validation, graph contribution APIs and identity snapshot projection; one narrow assembly module; offline native-planner and publication-image fixtures.
...
- Supplied projected images and domain observations are not proof of current physical sources/storage, semantic verification, accepted source ownership or dependency completeness.
- Existing graph wire/schema, contribution interpretation and retained-history projection remain unchanged; use the existing transformation owners.

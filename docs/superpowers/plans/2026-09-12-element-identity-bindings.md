...
2. Every target entity must already exist in the same spec. A non-null reference revision must exist and must be an active assessed revision at that historical point; its current head may since be terminal. Unbound references may be retained without fabricated content or a guessed current revision. Validate referenced namespace counter integrity and lifecycle head/revision bindings using the existing store checks.
...
- [ ] Add tests before corresponding behavior for unassessed claims, wrong/missing namespace or revision, terminal current targets with valid historical claims, reassessment retaining both revisions, atomic rollback on last invalid entry, conflicting/retried operation IDs, detached results, and numeric entry ordering.

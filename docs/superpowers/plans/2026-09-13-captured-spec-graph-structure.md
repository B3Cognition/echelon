...
- [ ] Compare the complete local structural subset from actual legacy build_spec_graph with the new fragment for real fixture trees, while exercising existing real parsers. External memory fixtures can retain the existing tests' deterministic adapter/audit stubs; do not mock the graph transformations being tested. Verify legacy full-graph memory/RE/topology tests unchanged in the covering set.

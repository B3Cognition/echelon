# Proposed identity history implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox syntax for tracking.

**Goal:** Preview the exact materialized history of a new proposed publication, so its graph can be rendered before artifact sealing without temporarily applying ledger writes.

**Architecture:** Compose existing operation validation, lifecycle planning and projected binding checks under one read-only IdentityStore transaction. Overlay validated planned rows on the existing full-audit history snapshot and encode the unchanged snapshot format. Share small operation/row construction rules with existing owners where necessary; do not add a second lifecycle or binding policy.

**Tech Stack:** SQLite query-only transactions, PublicationOperation/request codec, lifecycle/binding planners, IdentityHistorySnapshot, existing pure graph projection/rendering.
...
- No new database schema, implicit initialize/upgrade/import, allocation, persistent publication intent, lifecycle/binding mutation, provider activation or canonical graph/file write.
- Existing lifecycle planning, binding target validation, publication operation ownership and snapshot format remain authoritative. No transaction owner, rollback-based write simulation, memory database copy or parallel lifecycle validator is introduced.
- This is a pre-intent observation for new operations, not an idempotent publication receipt, semantic judgment, source authentication, namespace enrollment, reservation or future freshness guarantee.

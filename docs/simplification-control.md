...
| S4 | ACTIVE | Remove the abandoned strategy dimension and decompose single-run Delivery orchestration around its durable checkpoints. | Inventory proved every supported run uses the single built-in `default` path while `StrategyCoordinator._run_strategy` and `RalphController._run_loop_inner` each approach or exceed 1,000 lines. Hard-cutover design approved: `docs/superpowers/specs/2026-09-23-single-delivery-controller-design.md`. | Delivery has no strategy option or identity; one `DeliveryController` owns one run and Ralph performs one explicit durable step at a time; focused delivery suite passes. |
...
- [ ] Update current documentation and run focused Delivery verification.

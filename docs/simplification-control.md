...
| S4 | ACTIVE | Remove the abandoned strategy dimension and decompose single-run Delivery orchestration around its durable checkpoints. | Single-run cutover complete: focused partitions passed and the repository gate against `52f30d16` passed 9,830 tests with 0 failures. Delivery now has one controller, one `delivery.json`, and build-scoped artifacts. Next: write the checkpoint-led Ralph decomposition plan. | Delivery has no strategy option or identity; one `DeliveryController` owns one run and Ralph performs one explicit durable step at a time; focused delivery suite passes. |

...
| S4 | ACTIVE | Remove the abandoned strategy dimension and decompose single-run Delivery orchestration around its durable checkpoints. | Single-run cutover complete in focused verification: 354 artifact/recovery tests and 163 Ralph/controller tests passed. Delivery now has one controller, one `delivery.json`, and build-scoped artifacts. Next: write the checkpoint-led Ralph decomposition plan. | Delivery has no strategy option or identity; one `DeliveryController` owns one run and Ralph performs one explicit durable step at a time; focused delivery suite passes. |

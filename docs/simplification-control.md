...
| S4 | ACTIVE | Remove the abandoned strategy dimension and decompose single-run Delivery orchestration around its durable checkpoints. | Durable-step decomposition implemented: review re-entry partition 47 passed; combined Delivery/Ralph partition 259 passed; `_run_delivery` is 28 lines and `_run_loop_inner` is 159 lines. Repository gate pending. | Delivery has no strategy option or identity; one `DeliveryController` owns one run and Ralph performs one explicit durable step at a time; focused delivery suite passes. |

...
| S2 | ACTIVE | Make controlled delivery the sole supported delivery implementation. | Controller exists behind `llm.features.delivery_gate_controller`; default remains false. | Feature switch and feature-off path are removed; existing controlled-delivery, continuation, repair, verification, and review tests pass. |

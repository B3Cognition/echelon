...
| EGR-025 | P1 | open | Workflow transition validation still allows semantically wrong condition fields and can miss result/state contract drift. | The Phase 2 tracker bug used `alignment = ALIGNED OR alignment = DRIFT` in `extension/workflow/definition.yaml` while TRACKER emitted `echelon_result.verdict`; existing workflow validation passed because the condition was syntactically valid. | Extend workflow validation so transition condition fields must resolve to `verdict`, declared `allowed_state_updates`, known config-derived keys, or known derived state keys; add negative tests for unresolved condition fields. |
...
| P1 | EGR-025: Tighten workflow condition-field validation. | Prevents syntactically valid but semantically dead transitions from falling through at runtime. | `src/harness/workflow_validator.py`, `extension/workflow/definition.yaml`, `tests/kernel/test_workflow_validator.py` | Graph validation catches result/state contract drift before runs. |

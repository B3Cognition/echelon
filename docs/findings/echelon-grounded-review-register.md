...
| EGR-023 | P1 | open | Journal-entry validation is warn-then-allow, not strict, because prompt examples and agent outputs are not yet schema-complete. | `src/harness/journal_entry_validator.py` preserves invalid registered entries with `schema_warning`; `extension/scripts/bash/journal-append.sh` documents DR-001 warn-then-allow; `rg -n "journal_entries" extension/agents extension/workflow/phases` finds 114 prompt/spec mentions; tests still include sparse entries such as `{"type": "insight"}`. | Migrate agent and phase prompt examples to schema-complete `journal_entries.data` payloads, add static prompt/example validation against `extension/workflow/journal-entry-types.yaml`, then tighten Python writers from warning to blocking/quarantine for invalid registered types. |

## Next EGR Backlog

| Priority | Recommendation | Why it matters | Suggested files/modules to change | Expected impact |
|---|---|---|---|---|
| P1 | EGR-023: Tighten journal-entry schema enforcement after prompt migration. | Prevents schema warnings from becoming permanent journal pollution and closes the same class of loose-contract failure seen with early `echelon_result` formats. | `extension/agents/**/*.md`, `extension/workflow/phases/*.md`, `src/harness/journal_entry_validator.py`, `tests/kernel/test_squad_executors_journal.py`, new static prompt validation tests | Registered journal types become machine-checkable at generation time and invalid registered entries no longer silently enter durable memory. |

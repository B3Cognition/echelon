...
| EGR-012 | P1 | open | Pre-dispatch agent `state_updates` bypass per-phase allowlists. | `src/harness/squad_executors.py::_run_pre_dispatch()` writes `result.state_updates` directly via `state_store.save()` without calling `validate_echelon_result()` or `_validate_result_state_updates()`. | Validate pre-dispatch results before mutation; decide whether to use parent phase allowlists or per-pre-dispatch entry allowlists; add regression tests. |
| EGR-013 | P1 | open | COMMANDER judgment state updates bypass allowlists. | `src/harness/squad.py::_evaluate_transitions()` validates `next_phase` but applies extra `judgment.state_updates` directly; `_judgment_dispatch_escalation()` also applies returned updates directly with null-as-delete behavior. | Add a deterministic judgment-update allowlist and shared apply helper; preserve null-as-delete only for approved keys; test invalid judgment keys and banzai escalation cleanup. |
| EGR-014 | P2 | open | Allowed `state_updates` are enforced but not fully injected into agent prompts. | `src/harness/squad_executors.py::_routing_contract()` derives condition-required fields only; `_assemble_prompt()` and staged prompts do not render the full `node.allowed_state_updates` list from `definition.yaml`. | Add an explicit allowed-state-updates prompt section for normal, staged, conditional, and pre-dispatch prompts; test that empty allowlists are visible as `{}`. |

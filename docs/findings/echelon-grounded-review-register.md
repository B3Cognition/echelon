...
| EGR-026 | P1 | open | Verdict contracts are duplicated across workflow transitions, phase specs, agent prompts, and templates. | `phase2-tracker-alignment` had workflow outcomes `ALIGNED | DRIFT | STOP_AND_ASK`, while `extension/agents/control/tracker.md` and `extension/templates/intent-alignment-check-template.md` documented `ALIGNED | DRIFTING | ESCALATE`. | Add a static verdict-contract validator that compares transition verdict values against phase/agent/template declared enums, then migrate duplicated prompt text toward a single canonical declaration. |
...
| P1 | EGR-026: Add verdict-contract static validation. | Keeps workflow routing, phase specs, agent prompts, and templates from advertising different verdict enums. | New validator under `src/harness/`, `extension/workflow/phases/`, `extension/agents/`, `extension/templates/`, tests | Fewer hidden prompt/graph mismatches and fewer compatibility shims. |

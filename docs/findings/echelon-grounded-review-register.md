...
| EGR-021 | P2 | fixed | Extension/deployed-copy drift was a common operator footgun. | `src/harness/extension_drift.py`, `src/echelon/cli.py`, `README.md`, `tests/unit/test_extension_drift.py`, `tests/unit/test_cli_status.py` | Fixed: terminal `status`, `run`, `continue`, and `resume` warn when the installed `.specify/extensions/echelon` copy differs from the source checkout, while ignoring project-local config. |
...
| 2026-06-26 | `working tree on main` | EGR-021 added deterministic installed-extension drift detection for terminal CLI commands and documented the update path. Verification: `pytest tests/unit/test_extension_drift.py tests/unit/test_cli_status.py tests/unit/test_readme_tool_policy_docs.py -q` passed with 14 tests; `pytest tests/kernel -q` passed with 574 tests; `pytest -q` passed with 2403 tests and 22 skipped. |

...
| EGR-022 | P2 | in-progress | Core shell contract tests remain outside pytest collection. | `tests/run-all.sh`, remaining `tests/unit/*.sh`, `tests/integration/*.sh`, `tests/README.md`, plus migrated pytest coverage in `tests/unit/test_no_new_deps_pytest.py`, `tests/contract/no_new_deps.py`, `tests/unit/test_registry_sync_pytest.py`, `tests/contract/registry_sync.py`, `tests/unit/test_language_rules_pytest.py`, and `tests/contract/language_rules.py`. | Continue migrating deterministic unit-style shell checks to pytest; keep true shell integration tests as shell. |

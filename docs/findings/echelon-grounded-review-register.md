...
| EGR-024 | P1 | open | Prompt and example migration for journal entries is not yet statically enforced across all agent/phase files. | `rg -n "journal_entries" extension/agents extension/workflow/phases` found 114 prompt/spec mentions during the EGR-023 review; the canonical template is fixed, but scattered examples may still drift from `extension/workflow/journal-entry-types.yaml`. | Add static prompt/example validation against the journal-entry registry, migrate embedded examples to schema-complete `journal_entries.data`, and decide whether unknown unregistered types should remain allowed or be transformed into `unknown` records. |

## Next EGR Backlog

| Priority | Recommendation | Why it matters | Suggested files/modules to change | Expected impact |
|---|---|---|---|---|
| P1 | EGR-024: Statically validate journal-entry prompt/examples. | Prevents future prompt drift from reintroducing sparse or malformed journal entries after the runtime quarantine fix. | `extension/agents/**/*.md`, `extension/workflow/phases/*.md`, `extension/templates/echelon-result-template.yaml`, new static prompt validation tests | Registered journal types become machine-checkable at generation time, reducing runtime quarantine events. |

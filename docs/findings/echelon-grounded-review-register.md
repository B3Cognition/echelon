...
| EGR-009 | P3 | open | RCA pipeline is not implemented as a first-class capability. | No dedicated incident/RCA pipeline found under `src/` or `extension/workflow/`. | Remaining longer-term roadmap item: implement as a separate pipeline with incident intake, evidence collection, timeline reconstruction, hypothesis testing, corrective/preventive actions, and internalization update artifacts. |

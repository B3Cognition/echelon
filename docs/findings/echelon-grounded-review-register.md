...
| EGR-027 | P1 | open | `echelon continue` recovery can infer the first missing artifact producer instead of the next required workflow phase after a bad persisted state. | An already-affected run that fell through from `phase2-tracker-alignment` to `DONE` may have tracker alignment complete but no specialist outputs; `_next_continue_phase()` can choose `phase3-how` because HOW artifacts are absent, skipping `phase3-specialists`. | Add recovery detection for completed tracker alignment with missing specialist phase/output before HOW artifacts; route `continue` to `phase3-specialists` or provide an explicit supported rewind/repair path. |
...
| P1 | EGR-027: Harden `continue` recovery after bad persisted Phase A state. | Prevents recovery from skipping required workflow phases when old bugs have already written misleading `DONE` state. | `src/echelon/cli.py`, `tests/unit/test_cli_continue.py`, possibly docs for recovery guidance | Safer continuation for already-affected runs. |

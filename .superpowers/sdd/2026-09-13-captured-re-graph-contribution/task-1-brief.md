...
- All path values are canonical relative POSIX paths with no aliasing, absolute roots, traversal, backslashes or normalization changes. Reuse existing pure path validators plus exact string equality. The source-root fields workspace_path, semantic_path and topology.source_path additionally permit exact `.` through existing `normalize_source_root_path`, matching the valid workspace-root source; artifact and receipt paths do not. No `Path.resolve`, directory existence or symlink inference in this function. Source IDs use the existing pure RE source-ID policy after exact-string validation; admitting a root path does not admit source_id=`.` or change typed RE descriptor policy.
...
- [ ] Cover every public type/path/source-coherence/hash/generation/duplicate rule, receipt source_storage_key ownership, damaged records and nested containers, exception-chain removal and process-control propagation. Regression-first, exercise an actual legacy source with a valid ordinary RE source ID such as root and workspace/semantic/topology source paths `.`; assert existing SourceRoot path/title/receipt behavior before public reproduction. Retain rejection of dotted artifact/receipt paths and invalid RE source IDs. Use native storage-key behavior for valid inputs; do not mock it to invent a variant forbidden by the native RE source-ID policy. Add input/output damage after return, independently mutated subsequent results and no file/stat/resolve/enumeration/environment/process/clock/random/network/DB/memory/registry/identity/writer/lock tripwires after real fixture acquisition/imports. Do not mock pure transformations or assert only helper calls.

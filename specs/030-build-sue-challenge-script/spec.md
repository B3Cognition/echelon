...
- **Argv exposure of non-default provider protocols**: The default protocol keeps the prompt on stdin — specification text never enters process arguments. Argv-transport protocols added by later tiers (`copilot-argv`, v3.3) place the full prompt in argv: locally visible in process listings for the call's duration, and bounded by the OS argv limit (guarded at 200,000 characters with a named error). Do not use argv-transport providers for confidential specifications on shared machines.

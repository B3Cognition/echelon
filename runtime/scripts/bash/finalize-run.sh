...
CONSTITUTION_SRC="${PROJECT_ROOT}/.echelon/constitution.md"
CONSTITUTION_DST="${SPEC_DIR}/constitution.md"
if [ -f "${CONSTITUTION_SRC}" ]; then
  if grep -qE '\[[A-Z][A-Z0-9_]*\]' "${CONSTITUTION_SRC}"; then
    echo "[FINALIZE] ERROR: .echelon/constitution.md contains unresolved template markers"
    echo "[FINALIZE] Run echelon spec continue so CHIEF completes the constitution protocol before publishing the snapshot"
    exit 1
  fi
  cp "${CONSTITUTION_SRC}" "${CONSTITUTION_DST}"
  echo "[FINALIZE] constitution.md snapshot published from .echelon/ ✓"
else
  echo "[FINALIZE] ERROR: .echelon/constitution.md not found — cannot publish constitution snapshot"
  exit 1
fi

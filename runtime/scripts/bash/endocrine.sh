...
# Detect the active spec run from runs/.current.
...
agent_to_archetype() {
  ...
  case "$agent" in
    ...
    GATEKEEPER)
      echo "feasibility" ;;
    ARCHITECT|ORCHESTRATOR|SENTINEL|ORACLE|BENCHMARK|RE_PLANNER|RE_TASKER|RE_BASELINER|RE_DEEPENER|RE_RESOLVER|RE_SYNTHESIZER)
      echo "solution" ;;
    ...
    MIRROR|ADAPTIVE|AUDITOR|INTERNALIZER|REALIST|CONSOLIDATOR|VETERAN|MONITOR)
      echo "learning" ;;
    COMMANDER|SCOREKEEPER|TRACKER|STRATEGIST|PROGRESS_TRACKER|CHIEF|RE_CONSTITUTER|SUMMARIZER)
      echo "control" ;;
    *)
...
# Known agents list
ALL_AGENTS=(
  COMMANDER SCOUT SYNTHESIZER SAGE CARTOGRAPHER LEXICON_DERIVER MODELER
  ...
  RE_CHECKLISTER RE_VALIDATOR RE_VERIFIER RE_BASELINER RE_DEEPENER RE_RESOLVER RE_SYNTHESIZER
  SUMMARIZER
)

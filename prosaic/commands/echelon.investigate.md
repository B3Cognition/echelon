...
If `{{args}}` is empty or missing, report **"Please provide a question to investigate. Usage: speckit.echelon.investigate <your question>"** and stop.
...
- `.specify/extensions/echelon/templates/experiment-results-template.md` -- required format if an experiment runs
...
- **description:** "speckit-echelon-investigator: investigating -- {{{args}} truncated to 60 chars}"
...
- Add `"INVESTIGATOR"` to `active_specialists` if not already present
...
echelon_result:
  state_updates:
    active_specialists: <existing active_specialists plus INVESTIGATOR>
    updated_at: "{ISO-8601}"
  output_files: []
  journal_entries:
    - type: decision
      ...
      data:
        ...
        reasoning: "User explicitly requested SCIENTIST investigation for: {{{args}}}."
        rationale: "Dispatch INVESTIGATOR and record outputs under investigation/."
        content: "SCIENTIST dispatched for: {{{args}}}. See investigation/ for outputs."
```

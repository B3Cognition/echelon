...
def test_readme_and_catalog_match_agent_role_inventory() -> None:
    ...
    assert len(available) == 59
    assert len(dispatched) == 38
    assert len(direct_use) == 21
    assert missing == set()

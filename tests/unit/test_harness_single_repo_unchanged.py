...
class TestSingleRepoPathUnchanged:
    ...
    def test_targets_in_spec_dispatch_resume_before_workspace_config_validation(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """Polyrepo resume uses spec targets instead of requiring wrapper target_repo."""
        (tmp_path / ".git").mkdir()
        ...
        assert mock_orch.call_args.args[0] == "001-prose-distribution-engine"
        assert mock_orch.call_args.args[1] == [target.resolve()]
        assert mock_orch.call_args.args[2] == ["mode=banzai"]
        assert mock_orch.call_args.kwargs["command"] == "resume"
        assert mock_orch.call_args.kwargs["workspace_root"] == tmp_path.resolve()

    def test_target_side_resume_tolerates_workspace_config_without_target_repo(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """Target-dispatched resume gets target_repo from env, not workspace config."""
        from harness.config import ValidationError
        ...
        marker = current_build_marker(harness_base, "001-prose-distribution-engine")
        ...
        assert mock_cfg.call_args.kwargs["project_root"] == polyrepo
        assert mock_cfg.call_args.kwargs["squad_only"] is True
        MockGitOps.assert_called_once()
        assert MockGitOps.call_args.kwargs["base_dir"] == str(harness_base)
        assert mock_run.call_args.kwargs["base_dir"] == str(harness_base)
        assert mock_run.call_args.kwargs["config"].target_repo == str(target.resolve())
        assert mock_run.call_args.kwargs["config"].target_default_branch == "main"
        assert mock_run.call_args.kwargs["config"].provider == "docker"

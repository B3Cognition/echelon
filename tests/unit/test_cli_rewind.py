...
def _v3_gate_decision(
    *,
    status: str,
    decision_id: str,
    source_kind: str = "human_gate",
    source_phase: str = "checkpoint-assess",
    autonomy_mode: str = "banzai",
    ...
    legacy = build_blocked_decision_v2(
        ...
        selected_option_id="reject" if resolved else None,
        resolved_by="user" if resolved else None,
        attempts=2 if status == "failed" else 0,
        failure_code="resolution_attempts_exhausted" if status == "failed" else None,
        now="2026-08-23T10:00:00+00:00",
        resolved_at="2026-08-23T10:05:00+00:00" if resolved else None,
    )
    return validate_blocked_decision_v3(
        {
            **legacy,
            ...
            "recommendation_followed": False if resolved else None,
            "override_reason": None,
        }
    )


def _failed_decision_recovery(decision: dict[str, object]) -> dict[str, object]:
...
def _decision_display(decision: dict[str, object]) -> dict[str, object]:
    return {
        "escalation_question": decision["question"],
        "escalation_options": decision["options"],
        "escalation_resolved": decision["status"] == "resolved",
        "escalation_resolver": decision["resolved_by"],
        "escalation_selected_option": decision["selected_option_id"],
        "escalation_risk_level": decision["risk_level"],
        "escalation_recommended_answer": decision["recommended_answer"],
        "escalation_default_answer": None,
    }
...
def test_resolved_gate_rejection_rewind_preserves_terminal_authority(
    ...
    decision = _v3_gate_decision(
        status="resolved",
        decision_id="dec-resolved-gate-rewind",
    )
    run_dir = _write_run_state(
        tmp_path,
        {
            "run_id": "spec-20260618-073106-635192",
            "state_revision": 5,
            "status": "blocked",
            "phase": "terminal-blocked",
            ...
            "spec_id": "006-element-creator",
            "spec_dir": (
                "runs/spec-20260618-073106-635192/specs/006-element-creator"
            ),
            "completed_phases": ["phase1-lexicon"],
            "blocked_decision": decision,
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: RewindResult(
            applied=True,
            spec_id=spec_dir.name,
            checkpoint_id="phase1-lexicon",
            from_commit="b" * 40,
            to_commit="a" * 40,
            backup_ref="echelon/backup/resolved-gate",
            message="Rewind complete.",
        ),
    )

    _cmd_rewind(["phase1-lexicon", "--confirm"], project_root=tmp_path)

    rewound = json.loads((run_dir / "state.json").read_text(encoding="utf-8"))
    assert rewound["status"] == "running"
    assert rewound["phase"] == "phase1-lexicon"
    assert rewound["blocked_decision"] == decision
    assert "recovery_instruction" not in rewound
    for key, value in _decision_display(decision).items():
        assert rewound[key] == value


def test_failed_banzai_human_gate_rewind_retires_exact_authority_before_continue(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    decision = _v3_gate_decision(
        status="failed",
        decision_id="dec-failed-gate-rewind",
    )
    run_dir = _write_run_state(
        tmp_path,
        {
            ...
            "autonomy_mode": "banzai",
            "user_message": "review the Phase 1 boundary",
            "spec_id": "006-element-creator",
            "spec_dir": (
                "runs/spec-20260618-073106-635192/specs/006-element-creator"
            ),
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: RewindResult(
            ...
            from_commit="b" * 40,
            to_commit="a" * 40,
            backup_ref="echelon/backup/failed-gate",
            message="Rewind complete.",
        ),
    )
    ...
    assert rewound["status"] == "running"
    assert rewound["phase"] == "phase1-lexicon"
    assert rewound["state_revision"] == 6
    assert "blocked_decision" not in rewound
    assert "recovery_instruction" not in rewound
    assert not set(_decision_display(decision)) & set(rewound)
    ...
    assert calls == [["review the Phase 1 boundary", "--mode", "banzai"]]


@pytest.mark.parametrize(
    ("decision_updates", "checkpoint_next_phase"),
    [
        ({"source_kind": "provider_escalation"}, "checkpoint-assess"),
        ({"autonomy_mode": "semi"}, "checkpoint-assess"),
        ({"automatic_eligible": False}, "checkpoint-assess"),
        ({"status": "pending"}, "checkpoint-assess"),
        ({}, "checkpoint-plan"),
    ],
)
def test_confirmed_rewind_cannot_retire_mismatched_failed_gate_authority(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    decision = _v3_gate_decision(
        status=str(decision_updates.get("status", "failed")),
        decision_id="dec-mismatched-gate-rewind",
        source_kind=str(decision_updates.get("source_kind", "human_gate")),
        autonomy_mode=str(decision_updates.get("autonomy_mode", "banzai")),
        automatic_eligible=bool(
            decision_updates.get("automatic_eligible", True)
        ),
    )
    recovery = (
        RecoveryInstruction(
        ...
        else _failed_decision_recovery(decision)
    )
    run_dir = _write_run_state(
        tmp_path,
        {

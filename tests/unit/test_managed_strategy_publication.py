"""Reviewed strategy uses the guarded publisher and stops at alignment entry."""
...
from tests.unit.test_managed_strategy_execution import (
    case, enrolled, turn_prepared, prepared, checkpoint_case, StrategyExecutor,
    test_released_gate_authorizes_reviewed_strategy as complete_review,
)


def assert_strategy_publication(case, provider):
    ...
    with PhaseAExecutionLock.acquire(root, "test-strategy-publication"):
        with SpecRunExecutionLock.acquire(store.squad_dir, "test-strategy-publication"):
            package = prepare_discovery_publication(root, store, executor, completion_id="9" * 32, producer="strategy")
            recovery = json.loads(package.request.recovery_payload)
            assert recovery["version"] == 35 and recovery["producer"] == "strategy"
            binding = decode_binding(envelope(package), completion_id="9" * 32, state=before)
            ...
            assert set(writes) == {"specs/game/strategic-overview.md", "specs/game/spec-artifact-graph.json"}
...
def assert_closed_strategy_binding(case, package):
    before = case[1].load()
    for damage in ("version", "producer", "predecessor", "resolution", "scope", "routing", "review", "identity", "editable", "source"):
        recovery = json.loads(package.request.recovery_payload)
        if damage == "version": recovery["version"] = 31
        elif damage == "producer": recovery["producer"] = "feasibility"
        elif damage == "predecessor": recovery["predecessor"] = "strategy-" + "a" * 32
            ...
            if damage == "scope": candidate["artifacts"]["spec.md"] = "Unauthorized replacement"
        ...
        with pytest.raises(CompletionError):
            decode_binding(envelope(package, request), completion_id="9" * 32)
...
def assert_strategy_handoff(case, package, provider):
    ...
    before, history = store.load(), identity.identity_history(spec_id="game")
    documents = {p.name: p.read_bytes() for p in (root / "specs/game").iterdir() if p.is_file()}
    node = ctrl._graph.get("phase2-strategic-overview")
    with PhaseAExecutionLock.acquire(root, "test-strategy-handoff"):
        with SpecRunExecutionLock.acquire(store.squad_dir, "test-strategy-handoff"):
            ...
            result = SquadAgentResult(exit_code=0, echelon_result=dict(verdict="DONE", state_updates={}),
    ...
    after = store.load()
    assert after["phase"] == "phase2-tracker-alignment" and after["last_dispatch"]["post_dispatch_complete"] is True
    ...
    assert not after["phase_dispatch_counts"].get("phase2-tracker-alignment")
    assert not after["phase_dispatch_counts"].get("phase3-specialists")
    assert identity.identity_history(spec_id="game") == history
    assert identity.identity_publication(spec_id="game", operation_id="discovery-completion-" + "9" * 32)["state"] == "released"
...
def test_strategy_publishes_and_hands_off_without_running_alignment(checkpoint_case, provider, mode, enabled, policy):
    complete_review(checkpoint_case, provider, mode, enabled, policy)
    package = assert_strategy_publication(checkpoint_case, provider)
    assert_closed_strategy_binding(checkpoint_case, package)
    assert_strategy_handoff(checkpoint_case, package, provider)

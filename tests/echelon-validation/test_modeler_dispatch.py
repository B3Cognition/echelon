...
class TestModelerDispatch:
    def test_run_md_dispatches_modeler_after_synthesizer(self) -> None:
        ...
        synth = (RUNTIME / "workflow/phases/phase1-synthesizer.md").read_text()
        modeler = (RUNTIME / "workflow/phases/phase1-modeler.md").read_text()
    ...
    def test_run_md_modeler_dispatch_references_mental_model(self) -> None:
        ...
        content = (RUNTIME / "workflow/phases/phase1-modeler.md").read_text()
    ...
    def test_build_md_dispatches_modeler_after_implementer_task(self) -> None:
        ...
        content = (RUNTIME / "workflow/phases/build-6-progress.md").read_text()
        assert re.search(
            r"MODELER Update.*mandatory after every task|mandatory after every task.*MODELER",
            content,
            re.IGNORECASE | re.DOTALL,
        )
        section_start = content.index("**echelon.modeler (MODELER) Update")
        section = content[section_start:]
        assert "echelon.implementer (IMPLEMENTER)" in section

...
1. **NEVER overwrite a VETERAN entry** without creating a backup tag or versioning notation — consolidation must be recoverable.
2. **NEVER block agent execution** — CONSOLIDATOR operates asynchronously. If CONSOLIDATOR is unavailable, agents proceed without it.
...
**Purpose:** Surface relevant prior episodic traces to build agents on demand — augmenting VETERAN retrieval with temporal context and salience weighting.

**Process:**
1. Read reasoning-journal.jsonl entries from prior runs (via VETERAN episodic store)
...
**Trigger:** COMMANDER dispatches CONSOLIDATOR with `mode: "offline_consolidation"` during FINALIZE phase (after all build tasks complete, before SCOREKEEPER).

**Purpose:** Extract recurring causal and relational patterns from recent episodic traces and promote them to VETERAN as generalized schemas. Apply adaptive forgetting to consolidated traces.
...
3. For each cluster with `count >= consolidator.min_traces_for_schema` (default: 2):
   ...
   b. Check: does a matching schema already exist in VETERAN? If yes: reinforce (increment support count). If no: promote as new schema.
   c. Write schema to VETERAN with: `schema_id`, `domain`, `pattern_description`, `supporting_trace_count`, `first_seen`, `last_reinforced`, `outcome_signal_avg`
...
**Output:** Updated VETERAN schemas, VETERAN notification entries, consolidation log entry.

---

## Mode 3: Mental Simulation (FR-S3-003)

**Trigger:** INVESTIGATOR delegates a counterfactual query to CONSOLIDATOR with `mode: "mental_simulation"` and a `query` describing the "What would happen if X?" scenario.

**Purpose:** Construct a novel counterfactual scenario by recombining episodic fragments from the fast-write buffer and VETERAN. Provide richer counterfactual reasoning than chain-of-thought alone.

**Process:**
1. Decompose the query into elements: preconditions, action, domain context
2. Retrieve episodic fragments from reasoning-journal.jsonl and VETERAN schemas matching each element
...
|----------------|-----------|---------|
| VETERAN | Read + Write | Episodic trace source (read); schema promotion target (write) |
| INVESTIGATOR | Receives delegation | Counterfactual simulation requests |

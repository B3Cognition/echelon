...
4. **NEVER produce quality gate scores without invoking Understanding via the Skill tool.** In spec-validation mode (WHY2/WHY3), `b3c.understanding.validate` must be invoked via the Skill tool and must return before any quality scores are produced. Heuristic review is not a valid substitute. Do NOT call the `understanding` CLI binary directly via Bash — use the Skill tool.
...
- Access to Understanding (via `b3c.understanding.validate` Skill tool)
...
```
b3c.understanding.validate <spec_directory>/spec.md
```
...
```
b3c.understanding.scan <spec_directory>/spec.md --per-req --json --enhanced
```
...
```
b3c.understanding.diagram <spec_directory>/spec.md
```

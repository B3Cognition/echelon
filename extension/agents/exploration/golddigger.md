...
Small repos are cheap to extract at `full` depth in Mode 1, eliminating the need for Mode 2 dispatches on them entirely. Read the threshold from `echelon.yml` and auto-promote repos below it from `logic` to `full`.

```bash
THRESHOLD=$(grep 'polyrepo_full_depth_threshold' echelon.yml 2>/dev/null | awk '{print $2}' || echo 50)
```

...
| Ecosystem | Command |
| --- | --- |
| Node.js (npm/pnpm/yarn/bun) | `npm audit --audit-level=high 2>&1 \| tee /tmp/codegen-audit.txt \|\| { echo "✗ Security audit failed"; exit 1; }` |
...
- Any vulnerability found → `security_gate: "fail"` → record in `codegen-state.json`, print the finding, HALT. Do not auto-fix security vulnerabilities or license violations — escalate to human.

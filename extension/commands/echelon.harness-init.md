...
One-time setup that clones a mirror of the target repository, detects its language and Docker base image, and writes `.echelon/config.yml`. Legacy workspaces may still read `.specify/extensions/echelon/echelon-config.yml` during migration. Run this before any `harness.run` invocations.

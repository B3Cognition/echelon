...
Start with `--energy`. Then check extension config:

- If config `basic` is `true`, add `--basic` (skip NLP, still run energy)
- If config `per_req` is `true`, add `--per-req` (energy per requirement)
- If config `format` is `json`, add `--json` (structured energy data)
- User $ARGUMENTS flags always override config

### 3. Run Energy Analysis

```bash
understanding "$SPEC_PATH" --energy [FLAGS]
```

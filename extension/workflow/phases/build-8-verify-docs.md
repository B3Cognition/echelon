...
5. Return `echelon_result.verdict: PASS` when all documentation is adequate.
6. Return `echelon_result.verdict: FAIL` with structured repair findings when TECH WRITER must repair docs.
7. Return `echelon_result.verdict: BLOCKED` only when required inputs cannot be read.

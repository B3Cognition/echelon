...
**Quality gate sequence:** Quality gates are sequential hard gates, not a parallel batch. After implementation work completes, run SPEC GUARD first, then CODE REVIEWER, then TEST GUARDIAN, following `workflow/definition.yaml` transitions after each result. NEVER dispatch SPEC GUARD, CODE REVIEWER, and TEST GUARDIAN in one parallel batch. NEVER skip CODE REVIEWER or TEST GUARDIAN by vacuity. A gate may be skipped only when `workflow/definition.yaml` or a phase spec declares an explicit workflow-approved skip condition, and the skip rationale must be recorded in `echelon_result.journal_entries`.

...
sources: plan.md Risks, research.md ADRs, test-strategy.md.
...
|------|-------------|--------|-------|------------|
| T-005 subprocess runner | Medium | High | 6 | Invocation shape frozen by contracts/model-command-contract.md with Grade-A spike evidence (claude 2.1.214, ADR-003); recording stubs prove cwd/argv/stdin before any dependent task builds on it; sub-second timeout tests keep the failure mode visible in CI |
...
| Knowledge | ISS-201/ISS-203 counting wordings still unfixed in spec.md — literal test enumeration would assert wrong counts (e.g. AC-011 "2 content blocks") | Medium | Medium | Counting convention normatively pinned in contracts/model-command-contract.md (content block = data payload); T-004/T-014 test descriptions already adopt it; COMMANDER holds the CARTOGRAPHER rewording route |
